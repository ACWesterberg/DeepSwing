...
class Portfolio:
    ...
    def open_contract(
        ...
        """
        Open a binary event contract. Unlike open_trade there is no slippage or
        percentage commission: the price is already the ask we cross, and the fee
        is Kalshi's exact charge. All amounts are SEK.
        """
        ...
        position = OpenPosition(
            ...
            market="events",
            quantity=quantity,
            # Cost basis carries the fee, so pnl is net of it without any caller
            # having to remember; the raw ask is kept in entry_inputs.
            entry_price=price + (fee / quantity if quantity else 0.0),
            stop_loss=0.0,      # a binary settles at 0 or payout — no stop exists
            target=payout,
            entry_time=datetime.utcnow(),
            current_price=price,
            regime="event",
            ...
            confidence=confidence,
            entry_inputs=entry_inputs or {},
            last_news_price=price,
        )
        self.open_positions.append(position)
        self._next_trade_id += 1

        logger.info(
            "[%s] OPENED %.0f x %s @ %.4f (fee %.2f, payout %.4f)",
            self.track, quantity, ticker, price, fee, payout,
        )
        persist_portfolio(self)
        return position
    ...
    def settle_contract(
        ...
        """Resolve a binary contract at its settlement value. No slippage, no exit
        fee — Kalshi charges neither."""
        position = next((p for p in self.open_positions if p.trade_id == trade_id), None)
        if position is None:
            logger.warning("[%s] No open position with trade_id=%s", self.track, trade_id)
            return None

        self.cash += payout * position.quantity
        self.open_positions.remove(position)
        ...
        logger.info(
            "[%s] SETTLED %s at %.4f (P&L %.2f, %s)",
            self.track, position.ticker, payout, closed.pnl, exit_reason,
        )
        persist_portfolio(self)
        return closed

    def mark_positions(self, prices: dict[str, float]) -> None:
        """Mark open positions to market without any stop/target logic — binaries
        have neither, and the trailing stop in update_prices would close winners."""
