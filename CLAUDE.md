...
- **Option exits fill at the bid, marks stay at mid** — entry pays the ask (mid + adverse half-spread); a long exit receives the bid. Triggers (target/stop/time) are checked on the mid mark, filled at bid. Spread is the dominant option friction — mid-filling exits would hand back the half-spread paid at entry. Options strike selection uses a live spot with the same `max_entry_price_deviation` staleness guard as stock entries.
