...
- **Thinking models only for ERL** — standard models for the 30-min scan decisions (latency + cost); Claude Opus 4.8 with *adaptive* thinking + `output_config.effort` for post-trade causal analysis (deeper reasoning, async). The old `{"type":"enabled","budget_tokens":N}` shape returns a 400 on Opus 4.8, and `_call_claude` falls back to a plain call if the thinking request is rejected at all — a param change must never silently zero out heuristics.
...
- **A repeated question is not re-asked.** A candidate that clears the screener keeps clearing it, so the same ticker was sent to the decision model every scan — ~36 times a session at the old 15-min cadence — for an answer computed off *daily* bars. DSPy's own cache never catches this: the live price makes each prompt unique by a few decimals. `_pass_memo` in `scan_loop.py` reuses a stored **PASS** until it ages out (`decision_cache_minutes`, 60), the price moves `decision_recheck_move_pct` (1.5%), or the news+insider text changes. **PASS only** — a reused BUY would open a position against a stop and target placed at an older price, and BUYs are rare enough that re-asking costs nothing. The reused copy carries no `entry_inputs`, so the MIPRO/counterfactual corpus is untouched: both trainset queries filter on `entry_inputs IS NOT NULL`, and `_persist_decisions` already stored only one blob per track/ticker/day, so the day's first PASS still pays for its real call. `decision_cache_minutes=0` restores a call per scan. Likewise `analyze_news` caches on a hash of the article set (the *fetch* was already TTL-cached, so consecutive scans were buying the same summary); the jump-triggered exit review passes `use_cache=False` because freshness is what decides an exit there.
- **The stop sweep does not ride on the scan's timer.** `update_prices` books an exit at the price it *observes*, not at the level that was breached, so the gap between sweeps **is** the exit latency — and the sweep is price arithmetic, needing no candidates and no model. Bundling the two meant a cheap scan cadence bought a sloppy exit cadence: at `scan_interval_minutes` 30 a stop-out filled up to 30 minutes past the stop, ≈0.05–0.15R of extra loss on a 3%-of-price stop, for nothing (a real broker's stop order fills at the level whatever the poll rate). `holdings_interval_minutes` (15) now drives `run_holdings_monitor` on its own APScheduler job. It shares `_scan_lock` and skips a tick that lands mid-scan, because a scan ends with the same sweep. It adds no API spend: the news-exit review is gated on the move since the *last review*, not the last check, so checking more often detects that move sooner without adding calls — and the macro pull inside the sweep is lazy, so a pass with nothing to review costs one price fetch. `holdings_interval_minutes=0` folds it back into the scan. (`ai-fund-manager` splits `check-stops` from its weekly run the same way.)
...
src/scheduler/scan_loop.py  Main scan cycle (30-min): fetch → analyze → screen → decide → trade
...
main.py                     Entry point: DB init + APScheduler (scan / holdings sweep / watch / weekly) + uvicorn
```
